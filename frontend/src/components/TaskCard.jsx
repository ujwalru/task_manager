...
const TaskCard = ({ task, onDelete, onUpdate }) => {
  ...
  return (
    <div className="flex flex-col items-start p-4 my-2 bg-transparent rounded shadow-md border-2 border-emerald-500">
      ...
      <div className="flex">
        {editMode ? (
          ...
          <AiOutlineEdit className="text-2xl text-green-800 hover:text-green-300 mr-4" onClick={handleToggleEditMode} />
        )}
        <MdOutlineDelete className='text-2xl text-red-600 hover:text-red-800 ml-6' onClick={() => onDelete(task._id)} />
      </div>
    </div>
  );
};
